<?php
	...
	//$cEntities = $Entities->select(1, 'num_rows');
    $qEntities = $Entities->selectRequested(1, 'query');
    
    /*
    if(empty($cEntities))
        echo 'No existen entidades generadas para mostrar o bien se encuentran en estado inactivo!.';
    */
?>
...
?>
            <div class="bubble">
                <a href="<?=$row['host']?>" target="_blank">
                    <div class="l<?=(!empty($row['status'])) ? $row['status'] : 'ok'?>"></div>
                    <div class="c<?=(!empty($row['status'])) ? $row['status'] : 'ok'?>" title="<?=$row['host']?>">
                        <label><?=$row['entity']?></label>
                        <span><?='(' . $row['command'] . ')'?></span>
                    </div>
                    <div class="r<?=(!empty($row['status'])) ? $row['status'] : 'ok'?>"></div>
                </a>
            </div>
<?php
    }
