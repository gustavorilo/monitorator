<?php
  ...
  $arrClasses 	= array('CommandsTypes', 'Session');
	...
	$cantCommandT	= $CommandsTypes->select($search, 0, '', 'mynumrows');
	$qCommandT  	= $CommandsTypes->select($search, $offset, $byPage, 'query');
?>
<!doctype html>
...
?>
                        ...
                        <thead>
                        <tr>
                            <th class="uk-width-2-10">Comando</th>
                            <!--th class="uk-width-2-10 uk-text-center">Acciones</th-->
                        </tr>
                        </thead>
                        <tbody>
<?php
		while($row = $sqlCommon->myfetchassoc($qCommandT)) {
?>
                            <tr>
                                <td><?=$row['command']?></td>
                                <!--td class="uk-text-center">
                                    <a href="edit_commandtype.php?commandtype_id=<?=$row['idCommandType']?>"><i class="md-icon material-icons fa fa-pencil"></i></a>
                                    <!--a href="#" class="remove" id="circuits_<?=$row['id']?>"><i class="md-icon material-icons fa fa-remove"></i></a-->
                                <!--/td-->
                            </tr>
<?php
		}
?>                    
               	...
               	<?=$cantCommandT?> entidades encontradas.
<?php
		
		$url       = $_SERVER['PHP_SELF'] . '?action=l';
	  $url      .= ($search != '') ? '&search=' . $search : '';
	  $cantPages = ceil($cantCommandT / $byPage);
		
		if($cantCommandT > $byPage) {
...
?>
            		<div class="uk-overflow-container">
            			<?=($search != '') ? 'Término buscado: "<b>' . $search  . '</b>" &nbsp;&nbsp;|&nbsp;&nbsp; ' : ''?>
               		<?=$cantCommandT?> commandos encontrados.
            		</div>
<?php
	}
?>
